...
export default function SalesmanLedgerPage() {
  ...
  const loadSalesmanData = async () => {
    try {
      ...
      // Get current user to check if it's the same as the salesman
      const { data: { user: currentUser } } = await supabase.auth.getUser();
      
      // Set salesman info (use current user data if available, otherwise placeholder)
      ...
      setSalesmanInfo({
        ...
        email: isCurrentUser 
          ? (currentUser?.email || '')
          : `salesman-${profile.id}@system.local`,
