...
class BusinessController extends Controller
{
    ...
    */
    ...
    {
        if (! config('constants.allow_registration')) {
            return response()->json(['success' => false, 'msg' => 'Registration is disabled'], 403);
        }

        try {
            // Manually validate instead of using $request->validate() so we can catch validation errors
            ...
            if (config('constants.enable_recaptcha')) {
                ...
                if ($recaptcha_validator->fails()) {
                    return response()->json([
                        'success' => false,
                        'errors' => $recaptcha_validator->errors()
                    ], 422);
                }
            }
            ...
            // Set default values for optional tax fields
            $business_details['tax_label_1'] = empty($business_details['tax_label_1']) ? 'Tax 1' : $business_details['tax_label_1'];
            $business_details['tax_number_1'] = empty($business_details['tax_number_1']) ? '' : $business_details['tax_number_1'];
            $business_details['tax_label_2'] = empty($business_details['tax_label_2']) ? 'Tax 2' : $business_details['tax_label_2'];
            $business_details['tax_number_2'] = empty($business_details['tax_number_2']) ? '' : $business_details['tax_number_2'];
            
            // Set new business as active so user can login (is_active defaults to 1)
            ...
            if ($is_installed_superadmin && ! empty($package_id) && (config('app.env') != 'demo')) {
                ...
                if (! empty($package)) {
                    ...
                    return response()->json([
                        'success' => true,
                        'redirect' => route('register-pay', ['package_id' => $package_id]),
                        'msg' => __('business.business_created_succesfully'),
                    ]);
                }
            }

            // Success response - return JSON for AJAX form submission
            return response()->json([
                'success' => true,
                'redirect' => url('login'),
            ...
            \Log::emergency('File:'.$e->getFile().'Line:'.$e->getLine().'Message:'.$errorMessage);
