...
class CommonResourceController extends ApiController
{
     ...
     * @response {
        ...
        "data": [
            {
                "id": 1,
                "name": "DIN COLLECTION RABI",
                "landmark": "SHOP G139-140-141 RABI SADDAR ADAM G ROAD"
            },
            {
                "id": 2,
                "name": "NABEEL",
...
public function getDashboardSummary()
    {
        ...
        try {
            ...
            try {
                $sell_query = \App\Transaction::where('business_id', $business_id)
                    ->where('type', 'sell')
                    ->where('status', 'final');
                
                $today_sales = (float) ($sell_query->whereDate('transaction_date', date('Y-m-d'))
                    ->sum('final_total') ?? 0);
                    
                $total_sales = (float) ($sell_query->sum('final_total') ?? 0);
            } catch (\Exception $e) {
