...
/**
 * Settings Provider - Loads settings ONCE at root level
 * All components read from this memory-resident state (no localStorage reads per component)
 */
export function SettingsProvider({ children }: { children: ReactNode }) {
  const [settings, setSettings] = useState<SettingsData>(defaultSettings);
  const [loading, setLoading] = useState(true);

  // Load settings ONCE on mount - stored in memory
  ...
  useEffect(() => {
    ...
    // Listen for settings updates from Settings page (optimistic updates)
    const handleSettingsUpdate = (event: CustomEvent<SettingsData>) => {
      setSettings(event.detail);
      // Also update localStorage in background
      try {
        localStorage.setItem('studio_rently_settings', JSON.stringify(event.detail));
      } catch (error) {
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'studio_rently_settings' && e.newValue) {
        try {
  ...
  // Optimistic update function - updates memory state immediately
  const updateSettings = useCallback((newSettings: Partial<SettingsData>) => {
    setSettings((prev) => {
      ...
      if (typeof window !== 'undefined') {
        try {
          localStorage.setItem('studio_rently_settings', JSON.stringify(updated));
        } catch (error) {
...
/**
 * Hook to access settings from context (lightning fast - no localStorage reads)
 */
...
/**
 * Hook to get a specific setting value (selector pattern for performance)
 */
export function useSetting<K extends keyof SettingsData>(
  key: K,
  defaultValue?: SettingsData[K]
): SettingsData[K] {
